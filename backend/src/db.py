...
def get_members(db: Session, team_id: str) -> list[str]:
    return db.query(User.id).filter(User.team_id == team_id).all()
...
def calculate_team_scores(db: Session) -> list[TeamLeaderBoardRow]:
    ...
    subquery = (
        ...
        .filter(Submission.status == "AC")
        .group_by(Submission.team_id, Submission.problem_name)
        .subquery()
    )
    ...
    results = (
        ...
        .join(Team, Team.id == distinct_scores.c.team_id)
        .group_by(distinct_scores.c.team_id, Team.id, Team.name)
        .order_by(desc("total_score"), asc("total_submissions"))
        .all()
    )
    ...
    rank = 0
    prev_score = -1
    for team_id, team_id, team_name, total_score, total_submissions in results:
        if prev_score != total_score:
            rank += 1
        result.append(
            TeamLeaderBoardRow(
